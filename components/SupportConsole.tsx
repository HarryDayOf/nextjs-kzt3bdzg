...
function detectKeywords(text: string): { word: string; category: KWCategory }[] {
  const lower = text.toLowerCase();
  const hits: { word: string; category: KWCategory }[] = [];
  for (const [cat, data] of Object.entries(KEYWORD_CATEGORIES) as [KWCategory, typeof KEYWORD_CATEGORIES[KWCategory]][]) {
    for (const word of data.words) {
      if (lower.includes(word) && !hits.find(h => h.word === word)) {
        hits.push({ word, category: cat as KWCategory });
      }
    }
  }
  return hits;
    ...
    result = result.flatMap(node => {
      ...
      for (const word of data.words) {
        ...
        if (idx !== -1) {
          ...
          parts.push(<mark key={word + idx} style={{ backgroundColor: data.bg, color: data.color, fontWeight: 700, borderRadius: '3px', padding: '0 2px' }}>{remaining.slice(idx, idx + word.length)}</mark>);
...
type Message = { id: string; sender: string; sender_id: string; role: 'vendor' | 'couple'; text: string; ts: string; };
type Conversation = { id: string; participants: string[]; participant_ids: string[]; roles: ('vendor'|'couple')[]; listing: string; listing_id: string; txn_id: string | null; message_count: number; last_message: string; messages: Message[]; status: 'clean' | 'flagged'; reviewed: boolean; };

const MOCK_CONVERSATIONS: Conversation[] = [
  {
    id: 'conv_A1b2C3d4E5f6G7h8',
    participants: ['Marcus Webb', 'Sarah Chen'],
    participant_ids: ['b2e91d73-3f0c-4a8e-b654-9d7c1a3e5f28', '6f3a2c1e-84b7-4d9f-a312-1c8e5f2b7a04'],
    roles: ['couple', 'vendor'],
    listing: 'Full-Day Wedding Photography',
    listing_id: 'lst_4Hx9K2mPqR7vYnWd',
    txn_id: 'txn_3Ks9Lx2mPqR7vYnW',
    message_count: 6,
    last_message: '2025-01-18T10:42:00Z',
    reviewed: false,
    status: 'clean',
    messages: [
  ...
  {
    id: 'conv_B9c8D7e6F5g4H3i2',
    participants: ['Jordan & Priya Ellis', 'Bloom & Co Florals'],
    participant_ids: ['c7d45e81-2a9f-4b3c-96e7-5f1d8a0b2e34', 'a4f78c29-9e1b-4c6d-8a53-2f0e7b4d1c96'],
    roles: ['couple', 'vendor'],
    listing: 'Luxury Floral Design',
    listing_id: 'lst_2Rp6F4hQcM9yVjAg',
    txn_id: 'txn_8Fh4Tz3J7nLwS1uX',
    message_count: 8,
    last_message: '2025-02-08T14:17:00Z',
    reviewed: false,
    status: 'flagged',
    messages: [
      ...
      { id: 'm2', sender: 'Bloom & Co Florals', sender_id: 'a4f78c29', role: 'vendor', text: "Hi Jordan and Priya! For 150 guests we'd include 15 centerpieces, full ceremony arch, bridal party bouquets, and boutonnieres. The package price covers all of that.", ts: '2025-01-28T11:15:00Z' },
      ...
      { id: 'm6', sender: 'Bloom & Co Florals', sender_id: 'a4f78c29', role: 'vendor', text: "Totally fine! Let's proceed through the platform then. I'll update the listing with the cocktail hour add-on.", ts: '2025-02-02T09:05:00Z' },
  ...
  {
    id: 'conv_C4d3E2f1G8h7I6j5',
    participants: ['Marcus Webb', 'Bloom & Co Florals'],
    participant_ids: ['b2e91d73-3f0c-4a8e-b654-9d7c1a3e5f28', 'a4f78c29-9e1b-4c6d-8a53-2f0e7b4d1c96'],
    roles: ['couple', 'vendor'],
    listing: 'Garden Floral Package',
    listing_id: 'lst_7Tz3J8nLwS1uXkBe',
    txn_id: 'txn_1Rp5F4hQcM9yVjAg',
    message_count: 5,
    last_message: '2024-12-10T09:05:00Z',
    reviewed: true,
    status: 'flagged',
    messages: [
      ...
      { id: 'm2', sender: 'Bloom & Co Florals', sender_id: 'a4f78c29', role: 'vendor', text: "Hi Marcus! Happy to chat. Here's my personal email hello@bloomco.com — easier to coordinate off platform and I can send you a custom quote directly.", ts: '2024-12-05T11:30:00Z' },
  ...
  {
    id: 'conv_D5e4F3g2H1i8J7k6',
    participants: ['Jordan & Priya Ellis', 'Sarah Chen'],
    participant_ids: ['c7d45e81-2a9f-4b3c-96e7-5f1d8a0b2e34', '6f3a2c1e-84b7-4d9f-a312-1c8e5f2b7a04'],
    roles: ['couple', 'vendor'],
    listing: 'Elopement Photography',
    listing_id: 'lst_5Mv8G2tIpZ6xDrWq',
    txn_id: 'txn_6Mv7G2tIpZ4xDrWq',
    message_count: 4,
    last_message: '2025-02-21T16:30:00Z',
    reviewed: false,
    status: 'clean',
    messages: [
...
function Modal({ title, subtitle, children, onClose, wide }: any) {
  return (
    <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(15,20,40,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }} onClick={onClose}>
      <div style={{ backgroundColor: '#fff', borderRadius: '12px', boxShadow: '0 20px 60px rgba(15,20,40,0.2)', padding: '32px', width: wide ? '760px' : '580px', maxWidth: '96vw', maxHeight: '90vh', overflowY: 'auto' }} onClick={(e: any) => e.stopPropagation()}>
...
function ConversationModal({ conv, onClose, onReviewed, showToast, setActiveTab, setSearch }: any) {
  const allHits = conv.messages.flatMap((m: Message) => detectKeywords(m.text));
  const uniqueHits = allHits.filter((h, i, arr) => arr.findIndex(x => x.word === h.word) === i);
  const riskScore = Math.min(100, uniqueHits.length * 18 + (conv.status === 'flagged' ? 20 : 0));
  ...
  return (
    <Modal title="Conversation Thread" subtitle={conv.id} onClose={onClose} wide>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px', marginBottom: '20px' }}>
        {[
          { label: 'Participants', value: conv.participants.join(' + ') },
          { label: 'Listing', value: conv.listing },
          { label: 'Transaction', value: conv.txn_id ? <IdChip value={conv.txn_id} /> : '—' },
        ].map(({ label, value }) => (
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              ...
              <div style={{ width: '80px', height: '6px', backgroundColor: '#f3f4f6', borderRadius: '3px', overflow: 'hidden' }}>
                <div style={{ width: `${riskScore}%`, height: '100%', backgroundColor: riskColor, borderRadius: '3px', transition: 'width 0.3s' }} />
              </div>
            </div>
          </div>
          <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
            {uniqueHits.map(h => <KWChip key={h.word} word={h.word} category={h.category} />)}
          </div>
      ...
      <div style={{ backgroundColor: '#f9fafb', borderRadius: '10px', border: '1px solid #f3f4f6', padding: '16px', marginBottom: '20px', maxHeight: '340px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '12px' }}>
        {conv.messages.map((msg: Message) => {
          ...
          const hits = detectKeywords(msg.text);
          const hasFlaggedMsg = hits.length > 0;
          return (
            <div key={msg.id} style={{ display: 'flex', flexDirection: 'column', alignItems: isVendor ? 'flex-end' : 'flex-start' }}>
              ...
              <div style={{
                maxWidth: '75%',
                padding: '10px 14px',
                borderRadius: isVendor ? '12px 4px 12px 12px' : '4px 12px 12px 12px',
                backgroundColor: hasFlaggedMsg ? '#fdecea' : isVendor ? '#0f1428' : '#fff',
                border: hasFlaggedMsg ? '1px solid #fca5a5' : isVendor ? 'none' : '1px solid #e5e7eb',
                color: hasFlaggedMsg ? '#1f2937' : isVendor ? '#fff' : '#1f2937',
                fontSize: '13px',
                lineHeight: 1.6,
      ...
      <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
        {!conv.reviewed && <ActionBtn variant="success" label="Mark Reviewed" onClick={() => { onReviewed(conv.id); showToast('Conversation marked as reviewed.'); }} />}
        <ActionBtn label="View User" onClick={() => { onClose(); setActiveTab('users'); setSearch(conv.participants[1]); }} />
        <ActionBtn label="View Transaction" onClick={() => { if (conv.txn_id) { onClose(); setActiveTab('transactions'); setSearch(conv.txn_id); } }} />
        {conv.status === 'flagged' && <ActionBtn variant="danger" label="Suspend Vendor" onClick={() => showToast('Vendor suspended — action logged.')} />}
        {conv.status === 'flagged' && <ActionBtn variant="warning" label="Send Policy Warning" onClick={() => showToast('Policy warning sent to vendor.')} />}
      </div>
    </Modal>
  );
}
...
export default function SupportConsole({ user }: { user: any }) {
  ...
  const filteredConvs = conversations.filter(c => {
    const matchesFilter = convFilter === 'all' ? true : convFilter === 'flagged' ? c.status === 'flagged' : convFilter === 'clean' ? c.status === 'clean' : !c.reviewed && c.status === 'flagged';
    const matchesSearch = search ? c.participants.some(p => p.toLowerCase().includes(search.toLowerCase())) || c.listing.toLowerCase().includes(search.toLowerCase()) || c.id.toLowerCase().includes(search.toLowerCase()) : true;
    return matchesFilter && matchesSearch;
  });

  const markReviewed = (id: string) => setConversations(convs => convs.map(c => c.id === id ? { ...c, reviewed: true } : c));
    ...
    <div style={{ fontFamily: "'Inter', -apple-system, sans-serif", backgroundColor: '#f9fafb', minHeight: '100vh' }}>
      ...
      <header style={{ backgroundColor: '#fff', borderBottom: '1px solid #e5e7eb', padding: '0 32px', height: '58px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 50 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          {unreviewedCount > 0 && (
            <button onClick={() => { setActiveTab('conversations'); setConvFilter('flagged'); }} style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '5px 12px', backgroundColor: '#fdecea', border: '1px solid #fca5a5', borderRadius: '8px', fontSize: '12px', color: '#c62828', cursor: 'pointer', fontWeight: 600 }}>
      ...
      <main style={{ padding: '28px 32px' }}>
        ...
        {/* Search bar — not for conversations (has its own filter UI) */}
        ...
        {activeTab === 'conversations' && (
          <div style={{ marginBottom: '20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '10px' }}>
            ...
            <div style={{ position: 'relative' }}>
              ...
              <input style={{ paddingLeft: '30px', paddingRight: '12px', paddingTop: '7px', paddingBottom: '7px', width: '220px', backgroundColor: '#fff', border: '1px solid #e5e7eb', borderRadius: '8px', fontSize: '12px', color: '#374151', outline: 'none' }} placeholder="Search participants or listing..." value={search} onChange={e => setSearch(e.target.value)} />
            </div>
          </div>
        )}
          ...
          {activeTab === 'conversations' && (
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>{filteredConvs.map(c => {
                const allHits = c.messages.flatMap(m => detectKeywords(m.text));
                const uniqueHits = allHits.filter((h, i, arr) => arr.findIndex(x => x.word === h.word) === i);
                const riskScore = Math.min(100, uniqueHits.length * 18 + (c.status === 'flagged' ? 20 : 0));
                const riskColor = riskScore >= 60 ? '#c62828' : riskScore >= 30 ? '#f57f17' : '#2e7d32';
                return (
                  <tr key={c.id} style={{ cursor: 'pointer', backgroundColor: c.status === 'flagged' && !c.reviewed ? '#fffbf5' : 'transparent' }} onMouseOver={e => (e.currentTarget.style.backgroundColor = '#fafafa')} onMouseOut={e => (e.currentTarget.style.backgroundColor = c.status === 'flagged' && !c.reviewed ? '#fffbf5' : 'transparent')} onClick={() => setSelectedConv(c)}>
                    ...
                    <td style={td}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                        ...
                        <span style={{ fontSize: '11px', color: riskColor, fontWeight: 600 }}>{riskScore}</span>
                      </div>
                    </td>
                    <td style={td}>
                      {uniqueHits.length > 0
                        ? <div style={{ display: 'flex', gap: '3px', flexWrap: 'wrap' }}>{uniqueHits.slice(0, 3).map(h => <KWChip key={h.word} word={h.word} category={h.category} />)}{uniqueHits.length > 3 && <span style={{ fontSize: '11px', color: '#9ca3af' }}>+{uniqueHits.length - 3}</span>}</div>
